...
class WarehouseModel {
    ...
    static async list(page = 1, limit = 10, searchTerm = "") {
        ...
        if (searchTerm) {
            whereClause += " AND (name LIKE ? OR code LIKE ? OR warehouseType LIKE ? OR location LIKE ?)";
    ...
    static async generateNextCode() {
        const rows = await DatabaseUtils.query(
            `SELECT code FROM ${TABLE} WHERE code LIKE 'WH-%' ORDER BY id DESC LIMIT 1`
        );
        if (rows.length === 0) return 'WH-001';
        const lastCode = rows[0].code;
        const numPart = lastCode.replace('WH-', '');
        const nextNum = parseInt(numPart, 10) + 1;
        if (isNaN(nextNum)) return 'WH-001';
